...
export default function JoinScreen({ onJoin }) {
  ...
  const onKey = (e) => { if (e.key === 'Enter') handleJoin(); };
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-surface-50 via-brand-50 to-surface-100 p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-card border border-surface-200 p-8 animate-scale-in">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900 font-display tracking-tight">ChatBoat</h1>
          <p className="text-sm text-gray-400 mt-1 font-mono">Room-Based Messenger</p>
        </div>
        ...
        <div className="space-y-5">
          {/* Username */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2 font-mono">Username</label>
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all duration-150 bg-surface-50
              ...
              <FiUser className={`flex-shrink-0 ${errors.username ? 'text-red-400' : 'text-gray-400'}`} />
              <input
            ...
            {errors.username && <p className="text-xs text-red-500 mt-1 font-mono">{errors.username}</p>}
          </div>

          {/* IP / Room Code */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2 font-mono">
              IP Address <span className="text-gray-300 normal-case">(room code)</span>
            </label>
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all duration-150 bg-surface-50
              ...
              <FiWifi className={`flex-shrink-0 ${errors.ip ? 'text-red-400' : 'text-gray-400'}`} />
              <input
                type="text" maxLength={60}
                placeholder="e.g. 192.168.1.1"
                value={ip}
                onChange={e => { setIp(e.target.value); setErrors(p => ({...p, ip: ''})); }}
                onKeyDown={onKey}
            ...
            {errors.ip
              ...
              : <p className="text-xs text-gray-400 mt-1 font-mono">Anyone entering the same IP joins your room</p>
            }
          </div>
        ...
        <div className="flex items-center justify-center gap-4 mt-6 text-xs text-gray-400 font-mono flex-wrap">
          <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-400 inline-block" />No account needed</span>
          <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-brand-300 inline-block" />Same IP = same room</span>
        </div>
      </div>
    </div>
  );
